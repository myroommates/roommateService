...
public abstract class AbstractController extends Controller {
    ...
    /**
     * this function control the be.flo.roommateService.dto (via play.validation annotation) and return it if it's valid, or throw a runtimeException with an error message if not.
     */
    protected <T extends DTO> T extractDTOFromRequest(Class<T> DTOclass) {

        //extract the json node
        JsonNode node = request().body().asJson();
        //extract be.flo.roommateService.dto
        T dto = DTO.getDTO(node, DTOclass);
        if (dto == null) {
            throw new MyRuntimeException(ErrorMessage.JSON_CONVERSION_ERROR, DTOclass.getName());
        }

        //control be.flo.roommateService.dto
        try {
            validDTO(dto);
    ...
    private void validDTO(DTO dto) throws NoSuchMethodException, IllegalAccessException, InvocationTargetException {
        ...
        //control this object
        // validation(be.flo.roommateService.dto, lang());
                ...
                if (v instanceof DTO) {
                    ...
                    for (Object obj : ((Collection<?>) v)) {
                        ...
                        if (obj instanceof DTO) {
                            Logger.warn("               is a be.flo.roommateService.dto !! call validDTO...");
